<script>
  ...
  // Tukar data artikel dari pangkalan data ke format yang diperlukan oleh FoodCard
  ...
  onMount(() => {
    // Permulaan artikel dari data yang diambil di +page.js
  ...
  async function loadMoreArticles() {
    ...
    try {
      const response = await fetch(`/api/articles?websiteId=${data.website.id}&category=cafe&offset=${offset}&limit=${limit}`);
      const newArticles = await response.json();
      
      if (newArticles.length > 0) {
        articles = [...articles, ...newArticles.map(mapArticleData)];
        
        // Sesuaikan tinggi kad selepas artikel baru dimuat
        setTimeout(() => {
      ...
      console.error('Ralat memuat artikel tambahan:', error);
    } finally {
      loading = false;
    }
  }
...
<main class="bg-gradient-to-br from-gray-50 via-white to-red-50">
  ...
  <PageHeader 
    title="Artikel Cafe"
    description="Temukan kafe dan kedai kopi terbaik di Kelantan dengan ulasan lengkap dan rekomendasi terpercaya"
    icon="book"
    compact={true}
  />
  ...
  <section class="py-8 bg-gradient-to-r from-red-50 to-orange-50">
    <div class="container mx-auto px-4">
      <div class="max-w-2xl mx-auto text-center">
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Cari Artikel Cafe</h2>
        <p class="text-gray-600 mb-6">Temukan artikel, kafe, dan ulasan minuman kegemaran anda</p>
          ...
          <form on:submit|preventDefault={() => handleSearch()}>
            <div class="relative">
              <input
                ...
                placeholder="Cari artikel kafe, kedai kopi, atau ulasan minuman..."
